...
export function runTest() {
    ...
    assert(alice.rooms.has(techRoom) && alice.rooms.has(generalRoom) && alice.rooms.has(largeRoom));
    assert(bob.rooms.size === 2 && bob.rooms.has(techRoom) && bob.rooms.has(largeRoom));
    ...
    const roomUpdatesDisposer = autorun(() => {
        rooms.forEach(room => {
            if (room.roomUpdates.length > 0) {
                customLog(`[${room.name} Update] ${room.roomUpdates[room.roomUpdates.length - 1]}`);
            }
        });
    });
    ...
    customLog('[PASS] Verified that status updates are sent to all of Alice\'s rooms.');
    ...
    customLog('[PASS] Verified that status updates are only sent to Bob\'s rooms.');
