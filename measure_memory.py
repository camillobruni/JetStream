...
def get_color_for_value(v: float, max_v: float = 1024 * 1024) -> str:
  """Seamlessly interpolate between cold (blue) and hot (red) colors."""
  ratio = max(0, min(1.0, v / max_v))
...
def get_graph(history: List[Tuple[float, GCType]],
  ...
  if show_labels:
    label_max = f"{int(max_val // 1024):>4} MB ┐"
    label_min = f"{int(min_val // 1024):>4} MB ┘"
    label_mid = "        │"
    if width is None:
  ...
  if len(data) < width:
    padding_val = data[0][0] if data else 0
    data = [(padding_val, GCType.NONE)] * (width - len(data)) + data
...
def get_sparkline(history: List[Tuple[float, GCType]], width: int = 10) -> str:
  return get_graph(history, width=width, height=1, compress=True)[0]


def get_multi_line_graph(history: List[Tuple[float, GCType]],
                         height: int = 4) -> List[str]:
  return get_graph(history, height=height, show_labels=True, compress=False)
...
def main() -> None:
  ...
  print(f"{'Test Name':<33} {'Median':>12} MB ⌀ | "
        f"{'Peak':>4} MB ▲ | {'History':^10}"
  )
  print("-" * 80)
